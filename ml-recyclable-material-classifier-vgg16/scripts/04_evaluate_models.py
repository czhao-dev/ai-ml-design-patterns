...
CLASS_TO_LABEL = {"O": 0, "R": 1}
LABEL_TO_CLASS = {0: "O", 1: "R"}
...
def evaluate_model(model_path, model_name, images, images_scaled, labels):
    ...
    print(f"\n{model_name}")
    print(sk_metrics.classification_report(labels, predictions, target_names=["O", "R"]))
...
def main() -> None:
    ...
    print(f"Loaded {len(test_files)} test images: O={np.sum(labels == 0)}, R={np.sum(labels == 1)}")
    ...
    for model_path, model_name, fig_prefix in [
        ...
        plot_confusion_matrix(
            labels, predictions, labels=["O", "R"], title=f"{model_name} - Confusion Matrix"
        )
