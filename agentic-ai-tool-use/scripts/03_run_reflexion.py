#!/usr/bin/env python3
"""Run the Reflexion agent over the benchmark task set and save its metrics.

Usage:
    python scripts/03_run_reflexion.py [--limit N]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Reflexion agent over the benchmark task set.")
    ...
    eval_results = []
    for task in tasks:
        print(f"[reflexion] {task.task_id} ({task.category})...", end=" ", flush=True)
        ...
        reflections_note = f", {len(result.reflections)} reflection(s)" if result.reflections else ""
        print(
            f"{status} ({elapsed:.1f}s, {result.num_llm_calls} llm calls, "
            f"{result.num_tool_calls} tool calls{reflections_note})"
        )
    ...
    save_metrics(metrics, config.REFLEXION_METRICS_PATH)
    print(f"\nOverall success rate: {metrics['overall_success_rate']:.2%}")
    print(f"Estimated cost: ${metrics['estimated_cost_usd']:.4f}")
    print(f"Saved metrics to {config.REFLEXION_METRICS_PATH}")
