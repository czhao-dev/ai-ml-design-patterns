#!/usr/bin/env python3
"""Run the ReAct agent over the benchmark task set and save its metrics.

Usage:
    python scripts/01_run_react.py [--limit N]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the ReAct agent over the benchmark task set.")
    ...
    eval_results = []
    for task in tasks:
        print(f"[react] {task.task_id} ({task.category})...", end=" ", flush=True)
