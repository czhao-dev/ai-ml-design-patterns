...
from src.data_utils import extract_tar_archive, summarize_class_distribution
...
def extract_dataset() -> None:
    ...
    extract_tar_archive(DATASET_ARCHIVE, RAW_DATA_DIR)
...
def build_generators():
    ...
    return train_datagen, train_generator, valid_generator, test_generator
...
def plot_augmented_samples(train_datagen, save_path) -> None:
    ...
    sample_files = sorted((TRAIN_DIR / "crack").glob("*"))[:1]
    sample_img = tf.keras.preprocessing.image.img_to_array(
        tf.keras.preprocessing.image.load_img(sample_files[0], target_size=(150, 150))
    )
    sample_img = np.expand_dims(sample_img, axis=0)
    sample_label = np.array(["crack"])
...
def main() -> None:
    ...
    print("  valid:", summarize_class_distribution(VALID_DIR))
    print("  test: ", summarize_class_distribution(TEST_DIR))

    train_datagen, train_generator, valid_generator, test_generator = build_generators()
    ...
    print(f"  valid_generator: {valid_generator.samples} images, batches={len(valid_generator)}")
